...
import { Eye, Edit, Trash2, Search, User, Copy, Check } from "lucide-react";
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
              {loading ? (
                <tr><td colSpan="5" className="px-6 py-8 text-center text-slate-500">Loading students data...</td></tr>
                ...
                filteredStudents.map((student) => (
                  <tr key={student.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    <td className="px-6 py-4 flex items-center space-x-3">
                      ...
                      <div>
                        {/* Hover Copy Component for Name */}
                        <CopyableText text={student.fullName} />
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      {/* Hover Copy Component for ID */}
                      <CopyableText text={student.studentId} />
                    </td>
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-400">
                      Class {student.class} <span className="mx-2">•</span> Roll {student.rollNumber}
                    </td>
                    <td className="px-6 py-4">
                      {/* Hover Copy Component for Number */}
